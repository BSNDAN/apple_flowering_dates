...
def raster_to_img(rast):  
    """ Lire image tiff et la transformer en numpy array"""
...
def patch(data ,image, i, long):  
    """A partir de coordonnées d'un centre d'arbre, prends un patch de longueur 'long' """
    l = int(long/2)
...
def patch_to_list(capteur, trees):
    """ Prendre l'ensemble des patchs d'un capteur (MS/RGB) et les sotcker dans une liste"""
...
def tiles_to_list(capteur, data):
    ...
    """ Prends en entrée un capteur (RGB/MS) et va chercher les tiles correspondantes pour les stocker (month_img) ainsi que leur nom (id_tree), le numéro de tiles (identifiant) et le mois auquel elles correspondent"""
...
def tiles_df(capteur, split=2):
    """Pour un capteur donné (RGB/MS), renvoie un dataframe composé de tous les tiles"""
